    ...
    async start(controller) {
      ...
      try {
        ...
        const mediaNote = attached.length
          ? `\n\nThe user attached ${attached.length} file${attached.length === 1 ? "" : "s"} to this message. Use ${attached.length === 1 ? "it" : "them"} on the page — a picture or a video goes on whichever block it belongs to, with mediaUrl and mediaKind, and layout saying where it sits: stack, left, right, wide or full. A logo row goes on logos items as imageUrl. A media block is only for something standing entirely on its own. Set alt from the description. A photograph meant to sit behind the copy is a background instead: bgImageUrl on the section, with bgOverlay so the text stays readable, or theme bgImageUrl when it is meant to be behind the whole page. If the description says where it goes, that is where it goes. Use these URLs exactly as written; do not invent, shorten or re-host them, and never use a stock photo URL instead.\n\n${attached
              .map(
                (a) =>
